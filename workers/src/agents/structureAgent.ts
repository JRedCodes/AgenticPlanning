...
export async function runStructureAgent(message: string): Promise<StructureOutput> {
    const { object } = await generateObject({
        model: anthropic('claude-sonnet-4-6'),
        schema: StructureOutputSchema,
        prompt: `You are an expert software architect. Design a software system architecture as a node graph.
